...
impl Subsonic {
    ...
    pub async fn connect(&mut self) -> Result<(), Error> {
        if self.base_url.is_empty() {
            return Err(Error::msg("Subsonic server url is not configured"));
        }
    ...
    fn api_url(&self, endpoint: &str, params: &[(&str, &str)]) -> Result<Url, Error> {
...
impl Browsable for Subsonic {
    ...
    async fn artists(
        ...
        Ok(artists
            ...
            .take(normalize_limit(limit) as usize)
    ...
    async fn playlist(&mut self, id: &str) -> Result<Playlist, Error> {
        ...
        match response.playlist {
            ...
            None => Err(Error::msg("Playlist not found")),
        }
    }
...
impl From<Device> for Subsonic {
    fn from(device: Device) -> Self {
        ...
        Subsonic::with_credentials(&base_url, &username, &password)
    }
}
...
async fn request(url: Url) -> Result<ResponseBody, Error> {
    let mut response = surf::get(url.as_str())
        .await
        .map_err(|err| Error::msg(err.to_string()))?;
    if !response.status().is_success() {
        return Err(Error::msg(format!(
            "Subsonic request failed with status {}",
            response.status()
        )));
    }
    let envelope: SubsonicResponse = response
        .body_json()
        .await
        .map_err(|err| Error::msg(err.to_string()))?;
...
impl SubsonicResponse {
    pub fn into_result(self) -> Result<ResponseBody, Error> {
        ...
        if body.status != "ok" {
            ...
            return Err(Error::msg(format!("Subsonic error: {}", message)));
        }
        Ok(body)
    }
}
...
mod tests {
    ...
    fn live_navidrome_smoke() {
        ...
        runtime.block_on(async move {
            ...
            let tracks = client.tracks(None, 0, 50).await.expect("search3 failed");
            ...
            let mut response = surf::get(&track.uri).await.expect("stream request failed");
            assert!(
                response.status().is_success(),
                "stream url returned {}",
                response.status()
            );
            let body = response.body_bytes().await.expect("stream body failed");
            assert!(!body.is_empty(), "expected stream bytes");

            client.playlists(0, 50).await.expect("getPlaylists failed");
        });
    }

...
use anyhow::Error;
use music_player_types::types::{Album, Artist, CurrentPlayback, Device, Playback, Track};
use rust_cast::{
    channels::{
...
impl<'a> Chromecast<'a> {
    ...
    pub fn connect(device: Device) -> Result<(), Error> {
    ...
    pub fn reconnect(&mut self) -> Result<(CastDevice, String), Error> {
        let cast_device = match CastDevice::connect_without_host_verification(
    ...
    pub fn current_app_session(&mut self) -> Result<(CastDevice, String, i32, String), Error> {

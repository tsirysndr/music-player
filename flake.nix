    ...
    flake-utils.lib.eachSystem
      ...
      (
        ...
        let
          ...
          webuiFilter = path: _type: builtins.match ".*webui/musicplayer/build.*" path != null;
          schemaFilter = path: _type: lib.hasSuffix "extensions/schema.yaml" path;
          srcFilter =
            path: type:
          ...
          desktopSrc = lib.cleanSourceWith {
            src = ./.;
            filter = path: type: (desktopAssetFilter path type) || (srcFilter path type);
          };
          ...
          music-player = craneLib.buildPackage (
            commonArgs
            // {
          ...
          desktopArgs = commonArgs // {
            ...
            buildInputs = commonArgs.buildInputs ++ [ pkgs.freetype ] ++ desktopRuntimeLibs;
          };
          ...
          music-player-desktop = craneLib.buildPackage (
            desktopArgs
            // {
        ...
        in
        {
          checks = {
            ...
            music-player-clippy = craneLib.cargoClippy (
              commonArgs
              // {
                inherit cargoArtifacts;
